...
private:
    ...
    void InitializeBloodPressureInterface() {
        esp_err_t err = blood_pressure_host_.Init(BP_UART_PORT, BP_UART_TX_PIN,
                                                  BP_UART_RX_PIN, BP_UART_BAUDRATE);
        if (err != ESP_OK) {
            ESP_LOGW(TAG, "Failed to init blood pressure UART: %s",
                     esp_err_to_name(err));
            return;
        }

        blood_pressure_host_.OnVersion([this](uint8_t major, uint8_t minor,
                                              uint8_t patch, uint8_t model) {
            bp_version_.valid = true;
            bp_version_.major = major;
            bp_version_.minor = minor;
            bp_version_.patch = patch;
            bp_version_.model = model;
            ESP_LOGI(TAG, "BP version: %u.%u.%u (model 0x%02X)", major, minor,
                     patch, model);
        });
        ...
        blood_pressure_host_.OnResult([this](const Yp0xResult& result) {
            ...
            ESP_LOGI(TAG,
                     "BP measurement info=0x%02X SYS=%u DIA=%u MAP=%u PULSE=%u "
                     "EXT=0x%02X",
        ...
        mcp_server.AddTool(
            ...
            [this](const PropertyList&) -> ReturnValue {
                ...
                const esp_err_t query_err =
                    blood_pressure_host_.QueryLastMeasurement();
                if (query_err != ESP_OK) {
                    throw std::runtime_error(
                        std::string("查询测量值失败: ") +
                        esp_err_to_name(query_err));
        ...
        mcp_server.AddTool(
            "self.bp.get_cached_version", "获取已缓存的血压计版本信息(若有)",
            PropertyList(), [this](const PropertyList&) -> ReturnValue {
                ...
                snprintf(buffer, sizeof(buffer), "%u.%u.%u (model 0x%02X)",
                         bp_version_.major, bp_version_.minor, bp_version_.patch,
                         bp_version_.model);
                return std::string(buffer);
            });

        mcp_server.AddTool(
            "self.bp.get_cached_result", "获取已缓存的血压测量结果(若有)",
            PropertyList(), [this](const PropertyList&) -> ReturnValue {
                if (!blood_pressure_initialized_) {
                    throw std::runtime_error("血压串口未初始化");
                }
                ...
                snprintf(buffer, sizeof(buffer),
                         "info=0x%02X systolic=%u diastolic=%u mean=%u pulse=%u "
                         "extra=0x%02X",
                         result.info, result.systolic, result.diastolic,
                         result.mean, result.pulse, result.reserved);
                return std::string(buffer);
            });
    }
